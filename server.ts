...
io.on('connection', (socket) => {
  ...
  socket.emit('documents_updated', currentDocuments);
  socket.emit('codes_updated', currentCodes);
...
// Setup Firestore listeners on the server to sync with clients
onSnapshot(collection(firestoreDb, 'documents'), (snapshot) => {
  const docs: any[] = [];
  snapshot.forEach((doc) => docs.push(doc.data()));
  docs.sort((a, b) => new Date(b.submittedAt).getTime() - new Date(a.submittedAt).getTime());
  currentDocuments = docs;
  io.emit('documents_updated', docs);
});

onSnapshot(collection(firestoreDb, 'accessCodes'), (snapshot) => {
  let codeDocs: any[] = [];
...
app.patch("/api/documents/:id/status", async (req: any, res: any) => {
  try {
    const { id } = req.params;
    const { status } = req.body;
    await setDoc(doc(firestoreDb, "documents", id), { status }, { merge: true });
...
app.get("/api/documents", async (req: any, res: any) => {
  return res.json(currentDocuments);
});

app.get("/api/codes", async (req: any, res: any) => {
  return res.json({ codes: currentCodes });
});
